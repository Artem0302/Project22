...
class planets{
	static create(arr){
		let masplan=arr.map(el=>new planet(el))
		let planetsAccordion = masplan
			.map((el,index)=>el.renderPlanet(index))
			.join('');
		masplan.forEach((el,index)=>index==0 ? el.renderSolarSystem() : el.renderline(index));
...
class planet{
	...
	renderline(index){
      if (index!=0){
      	...
      	box.classList="box";
      	box.innerHTML=`<img class="image" id="${this.name.replace(' ','')}" src="./images/${this.name.replace(' ','')}.svg">`;
      	box.innerHTML+=`<p>${this.name.replace(' ','')}</p>`;
      	div.append(box);
      	let img=document.querySelector(`#${this.name.replace(' ','')}`);
      	img.width="30";
      	img.height="30";
	...
	renderSolarSystem(){
        header.innerHTML=`<img class="accordion-button__img" src="./images/${this.name.replace(' ','')}.svg" width="35" height="35">${this.name}`;
	}
}
planets.create(SolarSystemData);